class Shortcode():
    ...
    def run_atomic(self, pargs, kwargs, context):
        ...
        if "img2img_init_image" in self.Unprompted.shortcode_user_vars:
            ...
            init_img = self.Unprompted.shortcode_user_vars["init_images"][
                len(self.Unprompted.shortcode_user_vars["init_images"]) - 1]
        init_img_with_mask = self.Unprompted.shortcode_user_vars[
            "init_img_with_mask"] if "init_img_with_mask" in self.Unprompted.shortcode_user_vars else None

        self.Unprompted.log(self.Unprompted.shortcode_user_vars["script_args"])
        try:
            img2img_result = modules.img2img.img2img(
                "unprompted_img2img",  # id_task
                self.Unprompted.shortcode_user_vars["mode"] if "mode" in self.Unprompted.shortcode_user_vars else 0,
                # p.mode
                ...
                self.Unprompted.shortcode_user_vars[
                    "mask_blur"] if "mask_blur" in self.Unprompted.shortcode_user_vars else 0,  # p.mask_blur
                0.0,  # p.mask_alpha
                self.Unprompted.shortcode_user_vars[
                    "inpainting_fill"] if "inpainting_fill" in self.Unprompted.shortcode_user_vars else 0,  # p.inpainting_fill
                self.Unprompted.shortcode_user_vars["restore_faces"],
                self.Unprompted.shortcode_user_vars["tiling"],
                self.Unprompted.shortcode_user_vars["n_iter"] if "n_iter" in self.Unprompted.shortcode_user_vars else 1,
                # p.n_iter - batch count
                self.Unprompted.shortcode_user_vars[
                    "batch_size"] if "batch_size" in self.Unprompted.shortcode_user_vars else 1,  # p.batch_size
                self.Unprompted.shortcode_user_vars["cfg_scale"],
                self.Unprompted.shortcode_user_vars[
                    "image_cfg_scale"] if "image_cfg_scale" in self.Unprompted.shortcode_user_vars else None,
                self.Unprompted.shortcode_user_vars["denoising_strength"] if self.Unprompted.shortcode_user_vars[
                                                                                 "denoising_strength"] is not None else 1.0,
                ...
                self.Unprompted.shortcode_user_vars[
                    "resize_mode"] if "resize_mode" in self.Unprompted.shortcode_user_vars else 1,
                self.Unprompted.shortcode_user_vars[
                    "inpaint_full_res"] if "inpaint_full_res" in self.Unprompted.shortcode_user_vars else True,
                # p.inpaint_full_res
                self.Unprompted.shortcode_user_vars[
                    "inpaint_full_res_padding"] if "inpaint_full_res_padding" in self.Unprompted.shortcode_user_vars else 1,
                ...
                *self.Unprompted.shortcode_user_vars["script_args"]
            )
